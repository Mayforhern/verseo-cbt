...
import { Bell, Search, Clock, X, ThumbsUp, ThumbsDown, Copy, Send, Paperclip, Mic, Image, Globe, Save, Settings2, User, Mail, Lock, LogOut, Sun, Moon, Code, MessageSquare, Plus } from "lucide-react"
...
export default function AIChatHelper() {
  ...
  return (
    <div className="h-screen w-screen flex overflow-hidden bg-background">
      ...
      {/* Sidebar */}
      <aside
        className={`bg-card border-r border-border ${sidebarCollapsed ? "w-16" : "w-64"} flex flex-col transition-all duration-300 h-full`}
      >
      ...
      <main className="flex-1 flex flex-col bg-background h-full">
        ...
        <header className="border-b border-border p-4 flex items-center bg-card">
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <button className="text-muted-foreground hover:text-foreground transition-colors p-1.5 hover:bg-muted rounded-lg">
              <Bell size={18} />
            </button>
            <button 
            ...
            <span className="text-muted-foreground text-sm bg-muted px-2 py-0.5 rounded-lg">6/50</span>
          </div>
        </header>

        {/* Chat History Panel */}
        <div className={`fixed top-0 right-0 h-full w-96 bg-card border-l border-border transform transition-transform duration-300 ease-in-out ${
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-8">
              ...
              <h2 className="text-foreground text-xl font-semibold mb-2">Welcome to AI Chat Helper</h2>
            ...
            messages.map((message, index) => (
              <div key={index} className={`flex gap-4 ${message.role === 'assistant' ? 'justify-start' : 'justify-end'}`}>
                <div className={`flex max-w-[80%] ${message.role === 'assistant' ? 'flex-row' : 'flex-row-reverse'}`}>
                  <div className={`flex-shrink-0 ${message.role === 'assistant' ? 'mr-4' : 'ml-4'}`}>
        ...
        <div className="border-t border-border/50 p-4 bg-card">
          <div className="flex items-center gap-3 mb-3">
            <div className="flex items-center gap-2">
              <button className="text-muted-foreground hover:text-foreground p-2 hover:bg-muted/50 rounded-xl transition-all duration-200">
                <Paperclip size={18} />
              </button>
              <button className="text-muted-foreground hover:text-foreground p-2 hover:bg-muted/50 rounded-xl transition-all duration-200">
                <Image size={18} />
            ...
            <div className="flex-1">
              <div className="relative">
                ...
                <div className="absolute right-2 top-1/2 transform -translate-y-1/2 flex items-center gap-1.5">
                  <button className="text-muted-foreground hover:text-foreground p-1.5 hover:bg-muted/50 rounded-lg transition-all duration-200">
                    <Globe size={16} />
                  </button>
                  <button className="text-muted-foreground hover:text-foreground p-1.5 hover:bg-muted/50 rounded-lg transition-all duration-200">
                    <Settings2 size={16} />
            ...
            <button 
              ...
              className="text-white bg-gradient-to-r from-primary to-secondary rounded-xl p-2.5 hover:shadow-lg hover:shadow-primary/20 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Send size={18} />
            </button>
          </div>
